...
def handle_message(message):
    ...
    callback_button = types.InlineKeyboardButton(text="Меню №1 (борщь,каша,компот)", callback_data="1")
    keyboard.add(callback_button)
    callback_button = types.InlineKeyboardButton(text="Меню №2 (суп,каша,чай)", callback_data="2")
    keyboard.add(callback_button)
    callback_button = types.InlineKeyboardButton(text="Меню №3 (Молочная каша,кофе)", callback_data="3")
    keyboard.add(callback_button)
...
def callback_inline(call):
    if call.message:
        if call.data == "1":
            ...
            if 0 == session.query(User).filter(User.username == call.message.chat.first_name).count():
                add9 = User(username=call.message.chat.first_name)
                session.add(add9)
                session.commit()
                pass

            add91 = Buy(username=call.message.chat.first_name)
            session.add(add91)
            session.commit()
            add2 = call.message.text
            query = session.query(Buy)
            last = query.filter(Buy.buy_id).count()
            query = query.filter(Buy.buy_id == last)
            query.update({Buy.menu: add2})
            session.commit()
            ...
            if add2 == "Меню №3: Молочная каша,кофе":
                ...
                last = query.filter(Buy.buy_id).count()
                query = query.filter(Buy.buy_id == last)
                query.update({Buy.price: "24 грн"})
                session.commit()
                pass

            bot.send_message(call.message.chat.id, text="Укажите свой номер телефона в формате 0ххххххххх")
            pass
...
def echoall(message):
    if bool('0') == bool(re.match(r'0', message.text)):
        ...
        add3 = message.text
        query = session.query(User)
        last = query.filter(User.user_id).count()
        query = query.filter(User.user_id == last)
        ...
        last = query.filter(Buy.buy_id).count()
        query = query.filter(Buy.buy_id == last)
        query.update({Buy.data_city: add5})
        session.commit()
        pass
        ...
        last = query.filter(Buy.buy_id).count()
        query = query.filter(Buy.buy_id == last)
        query.update({Buy.data_time: add6})
        session.commit()
        ...
        query = session.query(Buy)
        last = query.filter(Buy.buy_id).count()
        calendar = session.query(Buy).get(last)
        calendar.calendar = add10
        ...
        menu = session.query(User).filter(User.user_id == last).one()
        ...
        and (bool(re.match(r'0', message.text)))):
...
def web_hook():
    bot.remove_webhook()
    bot.set_webhook(url='http://509b2de7.ngrok.io/' + config.token) #ngrok adress
